...
export const ImportService = {
  ...
  async initPdf() {
    ...
    try {
      ...
      console.warn("Sovereign PDF engine delayed.");
    }
  },

  sanitize(content: string): string {
    return content
      .replace(/__\d+\.__/g, '') // TRIPLE-PASS PURGE: Remove sharding artifacts
      .replace(/_\d+\._/g, '')   
      .replace(/\d+\.\s\_\_/g, ' ')
      .replace(/@\w+/g, ' ')
      .replace(/\n{3,}/g, '\n\n')
    ...
    try {
      ...
      switch (extension) {
        ...
        case 'docx':
          ...
          if (typeof conversionFn !== 'function' && typeof m.default === 'function') conversionFn = m.default;
          if (typeof conversionFn !== 'function') throw new Error("Conversion engine failure.");
        ...
        case 'pdf':
          ...
          break;
        default:
          throw new Error(`Format '${extension}' not bridged.`);
      }
  ...
  async extractPdfText(file: File): Promise<string> {
    ...
    if (!getDocument) throw new Error("PDF handshake failed.");

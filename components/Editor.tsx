...
import { Undo2, Redo2, Table } from 'lucide-react';
 ...
 */
export const Editor: React.FC<EditorProps> = ({ value, onChange, onFormat, font = 'mono', activeDocId }) => {
      ...
      <div className="h-10 px-4 flex items-center justify-between bg-obsidian-soft border-b border-vault-border text-[10px] uppercase tracking-widest text-vault-dim font-bold shrink-0">
        ...
        <div className="flex items-center gap-1">
          <button 
            onClick={handleUndo}
            disabled={pointer <= 0}
            className="p-1.5 hover:bg-vault-dim/5 rounded-md disabled:opacity-20 transition-all text-vault-dim hover:text-vault-text active:scale-90"
            title="Undo (Ctrl+Z)"
          >
            <Undo2 size={14} />
          </button>
          ...
          <button 
            onClick={onFormat}
            className="p-1.5 hover:bg-emerald-vault/10 rounded-md transition-all text-vault-dim hover:text-emerald-vault active:scale-90"
            title="Format Tables"
          >
            <Table size={14} />
          </button>

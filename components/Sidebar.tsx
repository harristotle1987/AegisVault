    ...
    <div 
      ...
      className={`group relative flex flex-col gap-1 rounded-2xl cursor-pointer transition-all duration-300 border active:scale-[0.98] ${
        ...
        isActive 
          ...
          : 'border-transparent text-vault-dim hover:bg-vault-dim/5 hover:text-vault-text'
      }`}
    >
        ...
        {(!isCollapsed || window.innerWidth < 768) && (
          <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity ml-2">
            <button 
              ...
              className="p-1.5 hover:text-emerald-vault text-vault-dim rounded-md hover:bg-vault-dim/5"
            >
              <Eye size={14} />
            </button>
            <button 
  ...
  return (
    ...
    >
      ...
      <div className="px-6 mb-6">
        <button 
          onClick={onCreate}
          className={`w-full flex items-center justify-center gap-3 py-3.5 rounded-2xl bg-emerald-vault hover:bg-emerald-vault/90 text-black transition-all group overflow-hidden active:scale-[0.98] shadow-lg shadow-emerald-vault/20 ${isCollapsed ? 'px-0' : 'px-4'}`}
        >
          <Plus size={20} strokeWidth={3} className="shrink-0" />
          {(!isCollapsed || window.innerWidth < 768) && <span className="text-[12px] font-black uppercase tracking-[0.25em]">Provision</span>}
        </button>
      </div>

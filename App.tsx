...
export default function App() {
  ...
  const initSpeech = useCallback(() => {
    if (typeof window !== 'undefined' && window.speechSynthesis) {
      ...
      window.speechSynthesis.onvoiceschanged = () => setSpeechReady(true);
    }
  }, []);
  ...
  const persistAudioProgress = useCallback((index: number) => {
    ...
    saveDraft({
      ...activeDoc,
      metadata: { ...activeDoc.metadata, audioProgress: index }
    });
  }, [activeDoc, saveDraft]);
  ...
  const playNextChunk = useCallback(() => {
    if (currentUtteranceIndex.current >= utterances.current.length) {
      ...
      persistAudioProgress(0);
      return;
    }
    const text = utterances.current[currentUtteranceIndex.current];
    const utter = new SpeechSynthesisUtterance(text);
    utter.rate = speechRate;
    utter.onstart = () => persistAudioProgress(currentUtteranceIndex.current);
    utter.onend = () => {
  ...
  const handleListen = useCallback((e?: React.MouseEvent) => {
    ...
    if (!speechReady) { initSpeech(); return; }
  ...
  return (
    <div className="fixed inset-0 overflow-hidden selection:bg-emerald-vault/30 flex flex-row bg-black text-white">
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden h-full">
        ...
        {/* Action Bank: QUADRANT-LOCKED FAR RIGHT */}
        <div 
          className={`fixed top-[18px] right-4 md:right-8 flex items-center gap-6 md:gap-10 z-[10002] transition-all duration-300 origin-right ${
            isSidebarOpen ? 'scale-75 translate-x-12 opacity-50' : 'scale-100'
          }`}
        >
           <button onClick={() => setActiveModal('scanner')} className="flex flex-col items-center gap-1 group">
             ...
             <span className="text-[8px] font-black uppercase tracking-[0.2em]">Scan</span>
           </button>
           <button onClick={handleListen} className="flex flex-col items-center gap-1 group">
             <div className={`p-2.5 border rounded-full transition-all ${isPlayingAudio ? 'bg-emerald-vault text-black' : 'bg-white/5 border-white/10 text-emerald-vault group-hover:bg-emerald-vault/20'}`}>
               {isPlayingAudio ? <BookOpen size={18} className="animate-pulse" /> : <Volume2 size={18} />}
             </div>
             <span className="text-[8px] font-black uppercase tracking-[0.2em]">Listen</span>
           </button>
        </div>
      ...
      {activeModal === 'scanner' && <ScannerOverlay onCapture={() => {}} onClose={() => setActiveModal(null)} />}
      <ConfigModal isOpen={activeModal === 'config'} onClose={() => setActiveModal(null)} docCount={documents.length} activeFont={activeFont} setActiveFont={setActiveFont} speechRate={speechRate} setSpeechRate={setSpeechRate} />
      <PurgeModal isOpen={activeModal === 'purge'} onConfirm={async () => { docToPurge && await deleteDraft(docToPurge); setActiveModal(null); }} onCancel={() => setActiveModal(null)} draftTitle="" />
      {activeModal === 'resume' && <ResumeAudioModal isOpen progress={activeDoc?.metadata.audioProgress || 0} total={utterances.current.length} onResume={() => startPlayback(activeDoc?.metadata.audioProgress)} onRestart={() => startPlayback(0)} onCancel={() => setActiveModal(null)} />}
      <ExportModal initialName={activeDoc?.title || ""} format={pendingFormat} onConfirm={async (name) => {
        if (!activeDoc || !pendingFormat) return;
        const fn = `${name}.${pendingFormat}`;
        let blob = pendingFormat === 'pdf' ? await VaultConverter.toPDF(activeDoc.content, fn, activeFont) : await VaultConverter.toDocx(activeDoc.content, fn);
        setLastExportedFile({ name, format: pendingFormat, blobUrl: URL.createObjectURL(blob) });
        setActiveModal('success');
      }} onCancel={() => setActiveModal(null)} />
      <DownloadSuccessModal isOpen={activeModal === 'success'} onClose={() => setActiveModal(null)} fileName={lastExportedFile.name} format={lastExportedFile.format} blobUrl={lastExportedFile.blobUrl} />
      <InstallPrompt />
    </div>
  );
}